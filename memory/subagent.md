...
## 主↔子文件传递
**原则**：subagent 自己管多步任务，**不进 plan_mode、不写 subagent_plan.md**（plan mode 已弃用，见 plan_sop 弃用banner）。主agent只看最终结果（output*.txt），不管内部怎么执行。
**文件传递机制**：主agent创建subagent时在task_dir中生成 `context.json`，包含所有文件的**绝对路径**
